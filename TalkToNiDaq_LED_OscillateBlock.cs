...
public class TalkToNiDaq_LED_OscillateBlock : MonoBehaviour
{
    [Header("Oscillating Block Cylinder Reference (auto-found if empty)")]
    public AnimateCylinderTextureOscillateBlock oscillateBlockTexture;

    [Header("LED Angle Ranges (0 to 360 degrees)")]
    [Tooltip("LED is ON when the oscillating azimuth falls within any of these ranges. Wrap-around is supported: e.g., from=330 to=30 means 330° through 0° to 30°.")]
    ...
    private void Start()
    {
        if (oscillateBlockTexture == null)
        {
            oscillateBlockTexture = FindObjectOfType<AnimateCylinderTextureOscillateBlock>();
            if (oscillateBlockTexture == null)
            {
                Debug.LogError("TalkToNiDaq_LED_OscillateBlock: No AnimateCylinderTextureOscillateBlock found in scene.");
                return;
            }
        }
    ...
    private void Update()
    {
        ...
        // Get current azimuth from oscillating block texture
        float azimuth = oscillateBlockTexture.AzimuthDeg;
        ...
        // ao2: LED based on oscillating azimuth
