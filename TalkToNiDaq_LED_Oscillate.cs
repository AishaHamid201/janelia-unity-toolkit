...
public class TalkToNiDaq_LED_Oscillate : MonoBehaviour
{
    [Header("Oscillating Cylinder Reference (auto-found if empty)")]
    public AnimateCylinderTextureOscillate oscillateTexture;
    ...
    private void Start()
    {
        if (oscillateTexture == null)
        {
            oscillateTexture = FindObjectOfType<AnimateCylinderTextureOscillate>();
            if (oscillateTexture == null)
            {
                Debug.LogError("TalkToNiDaq_LED_Oscillate: No AnimateCylinderTextureOscillate found in scene.");
                return;
            }
        }
    ...
    private void Update()
    {
        ...
        // Get current azimuth from oscillating texture
        float azimuth = oscillateTexture.AzimuthDeg;
