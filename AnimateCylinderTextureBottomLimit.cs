...
public class AnimateCylinderTextureBottomLimit : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (showDebugLog)
        {
            Debug.Log($"[AnimateCylinderTextureBottomLimit] Start: material={(cylinderMaterial != null ? "LOADED" : "NULL")}, " +
    ...
    void Update()
    {
        ...
        if (vel >= vRotDeg_per_sec.Length || vel >= sweepRepeatVec.Length)
        {
            if (showDebugLog && _debugFrameCount <= 5)
                Debug.Log($"[AnimateCylinderTextureBottomLimit] Frame {_debugFrameCount}: SKIPPED — vel={vel} >= array length (vRot={vRotDeg_per_sec.Length}, sweep={sweepRepeatVec.Length})");
            return;
        }
        ...
        if (cylinderMaterial & Time.time >= (waitTime+delaySeconds))
        {
            ...
            if (currentStep*vel <= (repeats * 2 * numElevationSteps * sweepRepeatVec[sweepRepeatVec.Length-1] * vRotDeg_per_sec.Length))
            {
                ...
                if (showDebugLog && _debugFrameCount <= 5)
                    Debug.Log($"[AnimateCylinderTextureBottomLimit] Frame {_debugFrameCount}: ROTATING — x={x:F4}, y={y:F4}, azimuth={_azimuthDeg:F1}°, vel[{vel}]={vRotDeg_per_sec[vel]}, step={currentStep}, dTime={dTime:F3}");
            ...
            {
                Debug.Log($"[AnimateCylinderTextureBottomLimit] Frame {_debugFrameCount}: STOPPED — currentStep*vel={currentStep*vel} > limit={repeats * 2 * numElevationSteps * sweepRepeatVec[sweepRepeatVec.Length-1] * vRotDeg_per_sec.Length}");
            }

        }
        else if (showDebugLog && _debugFrameCount <= 5)
        {
            Debug.Log($"[AnimateCylinderTextureBottomLimit] Frame {_debugFrameCount}: WAITING — material={(cylinderMaterial != null ? "OK" : "NULL")}, Time.time={Time.time:F3}, waitTime+delay={waitTime+delaySeconds:F3}");
        }
